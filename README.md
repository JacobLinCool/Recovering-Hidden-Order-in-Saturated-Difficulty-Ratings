...
This repository contains the manuscript, the code for its global and sequence experiments, and the frozen public evidence used in the paper.

## Contents

- `paper/manuscript/main.tex`, `references.bib`, the conference style files, the three figure sources or outputs, and `main.pdf` are the submission manuscript and build inputs.
- `paper/manuscript/artifact/public/` contains the released chart-level out-of-fold scores, aggregate comparisons, model configurations, per-fold sequence run status, and title-free Dan-i Dojo matches. `MANIFEST.json` records file hashes and sizes.
